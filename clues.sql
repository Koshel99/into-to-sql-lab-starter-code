...
SELECT CL.COUNTRYCODE, CL.LANGUAGE, CL.PERCENTAGE, C.NAME AS CountryName, C.CONTINENT, C.REGION FROM COUNTRYLANGUAGES CL LEFT JOIN COUNTRIES C ON CL.COUNTRYCODE = C.CODE WHERE CL.LANGUAGE = 'Italian' AND CL.PERCENTAGE = 100;
...
SELECT cities.*, cities.name AS city_name, countries.name, countries.region, countries.population, countries.code
FROM cities
